...
func (o *ChartJSOutput) Render(rows <-chan ch.Row, config any) error {
	...
	cOpts := chartjs.Options{
		Title:     cfg.Title,
		ScaleType: chartjs.NewScaleType(cfg.ScaleType),
		XLabel:    cfg.XLabel,
		YLabel:    cfg.YLabel,
		ZeroBased: cfg.ZeroBased,
		ColorType: chartjs.NewColorType(cfg.ColorType),
	}

	// Now use the legacy chartjs package
	c := chartjs.New(
		chartjs.NewChartType(cfg.ChartType),
		*ds,
		cOpts,
	)
	...
	if err := c.Build(chartjs.OutputAll, f); err != nil {
		return err
	}
